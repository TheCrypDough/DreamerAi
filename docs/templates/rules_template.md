# [CURSORAI PRE-PROCESSING INSTRUCTION - READ BEFORE FILLING TEMPLATE!]

**Objective:** Generate a detailed `rules_[agent_name].md` file for the specified agent using the template structure below.

**Mandatory Context Review:**
1.  **Identify Target Agent:** Determine the specific agent name (e.g., "Jeff", "Arch") for which this rules file is being created based on the current task context provided in the relevant `DreamerAi_Guide.md` Day entry.
2.  **Review Embedded Agent Description:** Locate and **thoroughly read** the detailed job description for the **TARGET AGENT** provided within the embedded "Agent Detailed Job Descriptions" text block below this instruction section.
3.  **Review Embedded Project Workflow:** **Thoroughly read** the embedded "Project Workflow" text block below this instruction section to understand the agent's interactions and place within the overall flow.
4.  **Synthesize & Apply:** Use the insights gained from the target agent's description and the project workflow context, combined with the specific V1/V2/V[X] scope and details defined in the relevant `DreamerAi_Guide.md` Day entry for this agent, to accurately and comprehensively fill out ALL sections in the subsequent template structure. Ensure the generated rules reflect the agent's specific responsibilities, communication lines, and current implementation scope/limitations accurately.

**[CURSORAI END PRE-PROCESSING INSTRUCTION. Embedded context follows, template structure starts after embedded context blocks.]**


**(BEGIN Embedded Agent Detailed Job Descriptions - Paste Full Text Here)**
...
**(END Embedded Agent Detailed Job Descriptions)**
---


**(BEGIN Embedded Project Workflow - Paste Full Text Here)**
...
**(END Embedded Project Workflow)**
---

**(BEGIN rules template structure)**

# Rules for [Agent Name Placeholder - CURSOR: Fill with Target Agent Name] ([Agent Role Placeholder - CURSOR: Fill based on Description/Guide Day])

## Role
[**CURSOR:** Synthesize a concise definition of the agent's primary function and archetype based *both* on its 'Core Responsibility' in the embedded description above *and* the specific focus of the current implementation version detailed in the relevant `DreamerAi_Guide.md` Day entry.]

## Scope (V[Version Placeholder - CURSOR: Fill based on Guide Day])
[**CURSOR:** Detail the specific responsibilities and boundaries for THIS version of the agent, drawing directly from the `DreamerAi_Guide.md` Day entry's Description/Tasks/Code. Reference the embedded description/workflow for broader context.]
*   Inputs Handled: [Specify inputs for THIS version]
*   Key Outputs/Artifacts: [Specify outputs for THIS version]
*   Specific Tasks Performed: [List actions for THIS version]
*   Key Limitations (V[X]): [List what THIS version DOES NOT do yet, referencing V2+/Future Vision]

## V2+ / Future Vision (Extracted/Summarized)
[**CURSOR:** Briefly summarize 2-4 key V2+/Future capabilities mentioned for the TARGET agent in the embedded description ('Future Potential' section).]

## Memory Bank (Illustrative Structure - Adapt per Agent)
[**CURSOR:** Provide 1-2 plausible, concrete examples for each item below based on the agent's role as defined in the embedded description and the V[X] scope.]
*   Key Input Context: [Example...]
*   Key Operational State: [Example...]
*   Key Output Context: [Example...]
*   Relevant Project Context: [Example...]
*   Last Updated: [YYYY-MM-DD HH:MM:SS Placeholder]

## Core Rules / Operating Principles (V[Version Placeholder - CURSOR: Fill based on Guide Day])
[**CURSOR:** List the fundamental rules governing THIS version's behavior. Synthesize from Guide Day implementation details, embedded Agent Description Core Rules, and Workflow interactions. Number them.]
1.  **Review Rules:** Read this file conceptually before complex tasks.
2.  **Use RAG:** [CURSOR: Specify IF and HOW this agent uses its RAG DB (`rag_[agent_name].db`) based on current implementation (Guide Day). E.g., "Query RAG DB via BaseAgent `query_rag` for X context."]
3.  **Use Assigned LLM:** [CURSOR: Specify IF and HOW this agent uses the LLM (e.g., "Use `self.llm.generate` via BaseAgent respecting `distill` flag / `agent_name` parameter.")]
4.  **Maintain Memory:** Add key inputs/outputs/states to `self.memory` via `add_message`.
5.  **Task Handoff/Communication:** [CURSOR: Detail how THIS version hands off tasks or communicates results (e.g., "Publish `event.name` via EventManager", "Call functional `route_tasks_n8n`", "Return dict to caller"). Reference embedded workflow/description.]
6.  **UI Communication:** [CURSOR: Detail how THIS version sends UI updates (e.g., "Use `self.send_update_to_ui` bridge call", "Publish status events for Dream Theatre via BaseAgent setter").]
7.  **Logging:** Use inherited `self.logger` for key actions, decisions, inputs, outputs, errors.
8.  **Error Handling:** Report errors clearly via return dictionary status/message and logger. Handle failures gracefully per implementation.
9.  **Collaboration:** [CURSOR: Specify any direct V1/V2 interactions implemented in this version (e.g., "Calls Riddick.run directly V1"). Reference embedded description/workflow.]
10. **[Agent Specific Rule 1]:** [CURSOR: Add specific constraint/logic from Guide Day implementation if applicable]
11. **[Agent Specific Rule 2]:** [CURSOR: Add specific constraint/logic from Guide Day implementation if applicable]

## Communication Protocols (V2+)
[CURSOR: Leave as placeholder V1, fill later based on Hermie V2+, Event system V2+, direct agent call protocols.]
- Target Agent: [e.g., Nexus] | Event/Method: [e.g., `nexus.task.request_review`] | Payload Structure: [e.g., `{'code_path': ..., 'task_id': ...}`] | Expected Response: [e.g., `{'status': 'accepted/rejected'}`]

**(End of Rules Template Structure)**