...
const https = __webpack_require__(/*! https */ "https"); // NEW - For secure token exchange request
const url = __webpack_require__(/*! url */ "url"); // NEW - For parsing redirect URL
const querystring = __webpack_require__(/*! querystring */ "querystring"); // NEW - For token exchange body
    ...
    tempServer = http.createServer(async (req, res) => {
      ...
      if (requestUrl.pathname === '/github_callback') {
        if (query.error) {
          ...
          if (rejectRedirectPromise && !handled) {
            handled = true;
            rejectRedirectPromise(new Error(errorMsg));
          }
          res.writeHead(400, {
          ...
          if (resolveRedirectPromise && !handled) {
            handled = true;
            resolveRedirectPromise(query.code);
          }
          res.writeHead(200, {
            'Content-Type': 'text/html'
          });
        ...
        res.writeHead(404);
        res.end('Not Found by Temp Server');
      }
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.handle('secure-jwt-save', async (event, token) => {
      console.warn("IPC: secure-jwt-save not implemented");
      return {
        success: false,
    ...
    ipcMain.handle('secure-jwt-get', async event => {
      console.warn("IPC: secure-jwt-get not implemented");
      return {
        success: false,
        token: null,
        error: 'Not implemented'
    ...
    ipcMain.handle('secure-jwt-delete', async event => {
      console.warn("IPC: secure-jwt-delete not implemented");
      return {
        success: false,
        error: 'Not implemented'
      };
    ...
    if (keytar) {
      ...
      ipcMain.handle('secure-keytar-save', async (event, {
        ...
        if (!secret) return {
          success: false,
          error: 'Cannot save empty secret.'
        };
        try {
      ...
      const keytarError = {
        success: false,
        error: 'Secure storage (keytar) unavailable.'
      };
    ...
    ipcMain.handle('start-github-auth', async () => {
      ...
      if (!githubCredentialsOk) return {
        success: false,
        error: 'GitHub credentials misconfigured.'
      };
      if (!keytar) return {
        success: false,
        error: 'Secure storage (keytar) unavailable.'
      ...
      try {
        ...
        return {
          success: false,
          error: error.message || "Unknown GitHub OAuth error."
        };
      }
    });
